...
app = Flask(__name__)
# si solamente mandamos a llamar a la clase y le pasamos la instancia de la clase Flask 
# creara los permisos para que todos puedan acceder (Allowed-Origin), 
# para que cualquier metodo pueda ser consultado (Allowed-Method) y para cualquier header (Allowed-Header)
# origin > '*' indicar que origenes pueden consumir mi API
# methods > ['GET','POST','PUT','DELETE','PATCH'] siempre se podra acceder al GET 
# pero si indicamos que solamente un metodo puede ser accedido entonces 
# solo se podra consultar el GET y ese metodo definido
# allow_headers > '*' indicar que cabeceran pueden ser enviadas a mi API
# Content-Type es un header que indicara que tipo de contenido estamos enviando , 
# cuando enviemos un json su valor sera 'application/json' , cuando enviemos un xml 'application/xml' 
# o cuando enviemos un archivo binario sera el formato de ese archivo 'image/jpeg'
...
@cross_origin(origins=["https://127.0.0.1:7000", 'https://mipagina.com'])
def obtener_clientes():
    # solamente puede ser llamado en cada controlador (funcion que se ejecutara cuando 
    # se realice una peticion desde el front)
    print(request.method)
    # request.method > mostrara el tipo de metodo al cual esta haciendo la consulta el front
    print(request.get_json())
    # request.get_json() > devolvera la informacion enviada por el body y la convertira a un diccionario 
    # para que python lo pueda entender y manipular
...
def gestion_usuario(id):
    ...
    if request.method == 'GET':
        ...
        if resultado is not None: 
            ...
            data = request.get_json()
            # en ese diccionario agregaremos una llave 'id' y almacenaremos el id del cliente 
            # que esta en la posicion 0 de la tupla del cliente encontrado
            data['id'] = id
        ...
        # eliminar ese cliente luego de validar si existe o no usando el metodo validar_usuario(id) 
        # si no existe indicar lo mismo 'cliente a eliminar no se encontro'
        resultado = buscar_usuario(id)
        if resultado:
            [cliente, posicion] = resultado
...
# debug > si esta habilitado (True) se reiniciara automaticamente el servidor cada vez que guardemos cambios 
